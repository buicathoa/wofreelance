  ...
  createCategory: async (req, res) => {
    ...
    try {
      ...
      for (let record of listRecords) {
        ...
        const getMethodPattern = RestApiMethods.get('wofreelance.jobcategories', 'all', ['id'])
        const promise = await sequelize.query(`select * from wofreelance.jobcategories where id = "${result[0]}"`)
